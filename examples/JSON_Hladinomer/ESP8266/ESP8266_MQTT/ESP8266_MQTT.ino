/*|-------------------------------------------------|*/
/*|Projekt: JSON client - ESP8266 - Hladinomer      |*/
...
// Root CA for webserver domain
const static char* webserver_root_ca PROGMEM = \
    "-----BEGIN CERTIFICATE-----\n" \
    ...
    "-----END CERTIFICATE-----\n";
#if defined HTTPS || defined MQTTS
X509List cert(webserver_root_ca);
#endif
// Root CA for MQTT Broker domain
const static char* mqtt_root_ca PROGMEM = \
    "-----BEGIN CERTIFICATE-----\n" \
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    // Attempt to connect
    if (client.connect("ESP8266_JSON_Client")) {
...
#if defined HTTPS
  Serial.printf("Using certificate for webserver: %s\n", webserver_root_ca);
  klient.setTrustAnchors(&cert);
#endif
#if defined MQTTS
  Serial.printf("Using certificate for webserver: %s\n", mqtt_root_ca);
  espClient.setTrustAnchors(&cert2);
#endif
#if defined MQTT
